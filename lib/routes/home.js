...
module.exports = function() {
  ...
  function extractLoginDataFromBody(body) {
    ...
    if (body.serverType === 'sentinel') {
      ...
      newConnection.sentinelName = body.sentinelName;
      ...
      client.on("connect", function () {
        client.call('info', 'keyspace', function(errCmd, serverInfo) {
          if (errCmd || !serverInfo) {
            ...
            client.call('config', 'get', 'databases', function(errCmd, serverInfo) {
                let dbMax = 16;
                let host = '';
                // ignore errors, often comamnd not allowed for security n stuff
                if (errCmd) {
                    console.info('Cannot query max number of databases allowed, use default 16 instead: ', errCmd.message);
                }
                else {
                    dbMax = Array.isArray(serverInfo) ? parseInt(serverInfo[1]) : 16;
                }
