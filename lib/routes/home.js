...
module.exports = function() {
  ...
  function getConfig (req, res) {
    req.app.getConfig(function (err, config) {
      ...
      // do not return password for configured connections...
      let reducedConfig = Object.assign({}, config);
      if (Array.isArray(reducedConfig.default_connections)) {
        reducedConfig.default_connections.forEach(function(item) {
          delete item.password;
        });
      }
      else {
        console.log('ERR: default_connections is no array: ' + (typeof reducedConfig.default_connections) + ' / ' +
          JSON.stringify(reducedConfig.default_connections));
        reducedConfig.default_connections = [];
      }
      return res.send(reducedConfig);
    });
  }
