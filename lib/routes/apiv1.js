...
module.exports = function (app) {
  ...
  routerV1.get('/server/info', getServerInfo);
  ...
  routerV2.get('/server/info', getServerInfo);
  routerV2.get('/server/:connectionId/info', getServerInfo);
  routerV2.get('/server/:connectionId/cluster/nodes', getClusterNodes);
...
function isConnected (req, res) {
  if (req.app.locals.redisConnections.getList()[0]) {
...
function getServerInfo (req, res, next) {
  ...
  if (res.locals.connection) {
    serverInfo(res.locals.connection, function (err, serverInfo) {
      if (err) {
  ...
  else if (req.app.locals.redisConnections.getList().length > 0) {
    let allServerInfo = [];
    // change from Array.forEach to async.each to not error out if one connection is not available atm!
    async.each(req.app.locals.redisConnections.getList(), function (redisConnection, callback) {
      serverInfo(redisConnection, function (err, serverInfo) {
        if (err) {
...
/** middleware to fetch the result of the redis command "cluster nodes" returned as an object
 *
 * @param req Express request object
 * @param res Express response object
 * @param next Express next middleware function
 */
function getClusterNodes (req, res, next) {
  if (res.locals.connection) {
    // only one server requested
...
async function serverInfo (redisConnection, callback) {
  ...
  try {
    ...
    connectionInfo.info = info.split('\n').map(function (line) {
      line = line.trim();
      let parts = line.split(':');
      if (parts[0] === 'cluster_enabled' && parts[1] === '1') isCluster = true;
      return {
    ...
    if (isCluster) {
      try {
        const clusterInfo = await redisConnection.cluster('info');
        const infos = clusterInfo.split('\n').map((line) => {
          line = line.trim();
          if (line) {
            let parts = line.split(':');
            return {
        ...
        const clusterInfoIdx = connectionInfo.info.findIndex((ci) => ci.key === 'Cluster enabled');
        connectionInfo.info.splice(clusterInfoIdx + 1, 0, ...infos);
      }
      catch (errCI) {
        // catch this here to return normal "info" output at least
        console.warn(`Error calling "cluster info" on cluster connection ${connectionInfo.connectionId}`, errCI);
      }
    }
    return callback(null, connectionInfo);
  }
  catch(errInfo) {
    console.error('getServerInfo', errInfo);
    connectionInfo.error = errInfo.message;
    return callback(errInfo, connectionInfo);
  }
}

function clusterNodes (redisConnection, callback) {
  redisConnection.cluster('nodes').then((cn) => {
    const nodes = cn.split('\n').map((line) => {
      ...
      return {
        ...
        aux: '',
        flags: line.shift(),
        primaryMaster: line.shift(),
        pingSent: line.shift(),
        pongReceived: line.shift(),
        configEpoch: line.shift(),
        linkState: line.shift(),
        slots: line.join(' ')
      }
    }).filter((n) => n.node !== undefined);
    return callback(null, nodes);
  }).catch((errCN) => {
    console.error('clusterNodes', errCN);
  ...
  redisConnection.keys(prefix, function (err, keys) {
    ...
    if (keys.length > 1) {
      keys = myutil.distinct(keys.map(function (key) {
        let idx = key.indexOf(redisConnection.options.foldingChar, prefix.length);
        if (idx > 0) {
...
function getKeysTree (req, res, next) {
  ...
  // for cluster setup need to query all nodes, not only one as for all other...
  let nodes;
  if (res.locals.connection.options.type === 'cluster') {
    nodes = res.locals.connection.nodes('master');
  }
  else {
    nodes = [res.locals.connection];
  }
  Promise.allSettled(nodes.map((node) => node.keys(search))).then(async (promises) => {
    ...
    // must be merged into "/blah" with:
    //      - two sub-key group "blub/" "blubber/"
    //      - two strings "yammy" "yo"
    let nodeResults = [];
    for (let idxPromise = 0; idxPromise < promises.length; ++idxPromise) {
      if (promises[idxPromise].status === 'rejected') {
        console.error(`getKeys failed for "${prefix}" on node ${idxPromise} (${nodes[idxPromise].options.host}:${nodes[idxPromise].options.port})`, promises[idxPromise].reason);
        return;
      }
      console.log(`found ${promises[idxPromise].value.length} keys for "${prefix}" on node ${idxPromise} (${nodes[idxPromise].options.host}:${nodes[idxPromise].options.port})`);

      let lookup = {};
      let reducedKeys = [];

      try {
        promises[idxPromise].value.forEach(function(key) {
          let fullKey = key;
          if (prefix) {
            key = key.substr(prefix.length);
          }
          let parts = key.split(redisConnection.options.foldingChar);
          let firstPart = '';
          ...
          if (key.startsWith(redisConnection.options.foldingChar) && !prefix) {
            parts.shift();  // remove empty first entry due to key starting with folding char
            firstPart = redisConnection.options.foldingChar;
          }
          firstPart += parts[0];
          if (parts.length > 1) {
            firstPart += redisConnection.options.foldingChar;
          ...
          if (lookup.hasOwnProperty(firstPart)) {
            ...
            lookup[firstPart] = {
              id: res.locals.connectionId + ":" + (prefix ? prefix : '') + firstPart,
              text: firstPart,
      ...
      for (let keyData of reducedKeys) {
        // add additional information for non-folders / full keys
        if (!keyData.children) {
          let type;
          try {
            type = await nodes[idxPromise].type(keyData.fullKey)
            keyData.rel = type;
          }
          catch (errType) {
            // log error and assign string for now but do not stop further key scanning
            console.warn(`Got error retrieving key type for ${keyData.fullKey} from ${nodes[idxPromise].options.host}:${nodes[idxPromise].options.port}: ${errType}`);
            type = 'string';
          }
          ...
          try {
            let count;
            if (type === 'list') {
              count = await nodes[idxPromise].llen(keyData.fullKey);
            }
            else if (type === 'set') {
              count = await nodes[idxPromise].scard(keyData.fullKey);
            }
            else if (type === 'zset') {
              count = await nodes[idxPromise].zcard(keyData.fullKey);
            }
            else if (type === 'stream') {
              count = await nodes[idxPromise].xlen(keyData.fullKey);
            }
            if (count) {
              keyData.text += ' (' + count + ')';
            }
            ...
            // just ignore size, nothing added than...
            console.warn(`Got error retrieving key size for ${type} ${keyData.fullKey} from ${node}: ${errSize}`);
          }
        }
        delete keyData.fullKey;

        // all data from first node are directly copied, all additional nodes must check if
        // this "folder" is already known and needs to be merged into result
        if (idxPromise > 0 && keyData.count !== 0) {
          const folder = nodeResults.find((item) => item.id === keyData.id);
          if (folder) {
            folder.count += keyData.count;
          }
          else {
            nodeResults.push(keyData);
          }
        }
        else {
          nodeResults.push(keyData);
    ...
    }).forEach(function(keyData) {
      // add count text if it is a "folder" (key prefix with multiple sub keys)
      if (keyData.count !== 0) {
        keyData.text += '* (' + keyData.count + ')';
        keyData.state = {
          opened: false
        };
      }
