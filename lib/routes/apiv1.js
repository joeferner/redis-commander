...
function getKeyDetailsList (key, req, res, next) {
  ...
  redisConnection.lrangeBuffer(key, startIdx, endIdx, function (err, items) {
    if (err) {
    ...
    items = items.map(function (item) {
      if (item[0].toString(16) === '1f' && item[1].toString(16) === '8b' && item[2].toString(16) === '8') {
        item = zlib.gunzipSync(item);
      }
      return {
        number: i++,
        value: item.toString()
