...
function postExec (req, res) {
  ...
  if (req.app.locals.redisReadOnly) {
    let commandNameUpper = commandName.toUpperCase();
    let commandNameSpace = commandNameUpper + ' ';
    if (!redisCoreCmds.readCmds.some(function(roCmd) {
      return roCmd === commandNameUpper || roCmd.startsWith(commandNameSpace);
    })) {
      return res.send('ERROR: Command not read-only');
    }
  }
  // check if known command to ioredis and execute with all params send
...
/** this method returns a list with all active redis commands that can be send
