  ...
  Promise.allSettled(nodes.map((node) => node.keys(search))).then(async (promises) => {
    ...
    for (let idxPromise = 0; idxPromise < promises.length; ++idxPromise) {
      if (promises[idxPromise].status === 'rejected') {
        console.error(`getKeys failed for "${prefix}" on node ${idxPromise} (${nodes[idxPromise].options.host}:${nodes[idxPromise].options.port})`, p.reason);
        return;
      }
      ...
      for (let keyData of reducedKeys) {
        ...
        if (!keyData.children) {
          ...
          try {
            ...
            console.warn(`Got error retrieving key type for ${keyData.fullKey} from ${node}: ${errType}`);
            type = 'string';
          }
