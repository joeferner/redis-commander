...
function getKeysTree (req, res, next) {
  ...
  Promise.allSettled(nodes.map((node) => node.keys(search))).then((promises) => {
    ...
    let nodeResults = promises.flatMap((p, idx) => {
      if (p.status === 'rejected') {
        console.error('getKeys', p.reason);
        return [];
      }
      console.log(`found ${p.value.length} keys for "${prefix}" on node ${idx} (${nodes[idx].options.host}:${nodes[idx].options.port})`);

      let lookup = {};
      let reducedKeys = [];

      try {
        p.value.forEach(function(key) {
          let fullKey = key;
          if (prefix) {
      ...
      reducedKeys.map(async (keyData) => {
        if (!keyData.children) {
          let type;
          try {
            type = await nodes[idx].type(keyData.fullKey)
            keyData.rel = type;
          }
          catch (errType) {
          ...
          try {
            let count;
            if (type === 'list') {
              count = await nodes[idx].llen(keyData.fullKey);
            }
            else if (type === 'set') {
              count = await nodes[idx].scard(keyData.fullKey);
            }
            else if (type === 'zset') {
              count = await nodes[idx].zcard(keyData.fullKey);
            }
            else if (type === 'stream') {
              count = await nodes[idx].xlen(keyData.fullKey);
            }
            if (count) {
              keyData.text += " (" + count + ")";
            }
