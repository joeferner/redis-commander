...
const ControlCharRegEx = new RegExp('[\0-\x1F\x7F-\x9F]');
...
function getKeyDetailsString (key, res, next) {
  ...
  redisConnection.get(key, function (err, val) {
    ...
    // simple check if contains POSIX reqex group of control chars
    // ref for possible groups https://github.com/slevithan/xregexp/blob/master/tools/output/categories.js
    if (ControlCharRegEx.test(val)) {
      details.type = 'binary';
      details.value = new Buffer(val).toString('base64');
    }
