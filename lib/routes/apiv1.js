...
module.exports = function (app) {
  ...
  app.get('/apiv1/key/:connection/:key/:index?', getKeyDetails);
...
function getConnection(redisConnections, connectionId, callback) {
  ...
  var desiredHost = hostAndPort[0];
  var desiredPort = hostAndPort[1];
  redisConnections.forEach(function(connection) {
    if (connection.host === desiredHost && connection.port === desiredPort) {
...
function getServersInfo (req, res, next) {
  if (req.redisConnections.length > 0) {
    //TODO: LOOP THROUGH  ALL REDIS CONNECTIONS AND RETURN ALL SERVER INFO
    var allServerInfo = [];
...
function getKeyDetails (req, res, next) {
  var connectionId = decodeURIComponent(req.params.connection);
  ...
  getConnection(req.redisConnections, connectionId, function (err, redisConnection) {
    ...
    redisConnection.type(key, function (err, type) {
      ...
      switch (type) {
        case 'string':
          return getKeyDetailsString(key, redisConnection, req, res, next);
        case 'list':
          return getKeyDetailsList(key, redisConnection, req, res, next);
        case 'zset':
          return getKeyDetailsZSet(key, redisConnection, req, res, next);
        case 'hash':
          return getKeyDetailsHash(key, redisConnection, req, res, next);
        case 'set':
          return getKeyDetailsSet(key, redisConnection, req, res, next);
      }
...
function getKeyDetailsString (key, redisConnection, req, res, next) {
...
function getKeyDetailsHash (key, redisConnection, req, res, next) {
...
function getKeyDetailsSet (key, redisConnection, req, res, next) {
...
function getKeysTree (req, res, next) {
  ...
  req.redisConnections[0].keys(search, function (err, keys) {
    if (err) {
      console.error('getKeys', err);
      return next(err);
    }
    ...
    async.forEachLimit(reducedKeys, 10, function (keyData, callback) {
      if (keyData.leaf) {
        req.redisConnections[0].type(keyData.fullKey, function (err, type) {
          if (err) {
            return callback(err);
          }
