...
module.exports = function (app, urlPrefix) {
  ...
  router.post('/editHashRow', postEditHashFieldOld);

  // modify entries - newer api with post/put/del and additional POST equivalents
  // and unified form params
  router.post('/hash/field', postAddHashField);
  router.put('/hash/field', postEditHashField);
  router.delete('/hash/field', postDeleteHashField);
  router.post('/hash/field/add', postAddHashField);
  router.post('/hash/field/edit', postEditHashField);
  router.post('/hash/field/delete', postDeleteHashField);
  ...
  router.post('/exec/:connectionId', postExec);
...
function getKeysTree (req, res, next) {
  ...
  redisConnection.keys(search, function (err, keys) {
    ...
    keys.forEach(function (key) {
      ...
      if (key.startsWith(foldingCharacter) && parts.length > 1) {
        firstPrefix = foldingCharacter + parts[1];
      }
