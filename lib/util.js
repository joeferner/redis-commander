...
function validateConfig() {
  ...
  // a comma separated string is allowed too to be able to set more than one algorithm via environment variable
  if (! Array.isArray(c.get('sso.jwtAlgorithms'))) {
    const algorithms = String(c.get('sso.jwtAlgorithms')).split(',').map((alg) => alg.trim()).filter((alg) => alg);
    if (algorithms.length === 0 || algorithms.some((alg) => (alg.toLowerCase() === 'none'))) {
      console.warn('Attention - insecure "none" algorithm allowed to check external SSO JWT token');
    }
    c.util.setPath(c, ['sso', 'jwtAlgorithms'], (algorithms.length ? algorithms : ""));
  }

  // check keys used to validate the signature of external SSO JWT tokens
  if (c.get('sso.enabled')) {
    convertNumbers('sso.jwksCacheMaxAge');
    validateNumbers('sso.jwksCacheMaxAge', true, 0, Number.MAX_VALUE);

    const keySources = ['sso.jwtSharedSecret', 'sso.jwtPublicKey', 'sso.jwksUri'].filter((key) => !!c.get(key));
    if (keySources.length === 0) {
      hasError('SSO login is enabled but no key to validate the JWT signature configured. Set one of the config keys "sso.jwtSharedSecret", "sso.jwtPublicKey" or "sso.jwksUri"');
    }
    else if (keySources.length > 1) {
      hasError(`Only one key to validate SSO JWT signatures allowed but more than one configured: ${keySources.join(', ')}`);
    }
    else if (keySources[0] !== 'sso.jwtSharedSecret') {
      // public keys must never be accepted for HMAC signatures - everybody knowing the public key
      // (it is public...) could use it as shared secret to sign tokens on his own
      const algorithms = (Array.isArray(c.get('sso.jwtAlgorithms')) ? c.get('sso.jwtAlgorithms') : []);
      const hmacAlgorithms = algorithms.filter((alg) => String(alg).toUpperCase().startsWith('HS'));
      if (algorithms.length === 0) {
        hasError(`Config key "sso.jwtAlgorithms" must list the allowed signature algorithms (e.g. "RS256") if "${keySources[0]}" is used`);
      }
      else if (hmacAlgorithms.length > 0) {
        hasError(`Config key "sso.jwtAlgorithms" must not contain the HMAC algorithms ${hmacAlgorithms.join(', ')} if "${keySources[0]}" is used - use asymmetric algorithms like "RS256" instead`);
      }
    }

    const jwksUri = c.get('sso.jwksUri');
    if (jwksUri) {
      let protocol = '';
      try {
        ...
        hasError(`Config key "sso.jwksUri" is not a valid url (current: "${jwksUri}")`);
      }
      if (protocol === 'http:') {
        console.warn('Attention - unencrypted http url used to download the keys to validate external SSO JWT token');
      }
      else if (protocol && protocol !== 'https:') {
        hasError(`Config key "sso.jwksUri" must be a http or https url (current: "${jwksUri}")`);
      }
    }

    // load key here already to fail on startup and not on first login attempt
    if (c.get('sso.jwtPublicKey')) {
      try {
        require('./ssoKeys').loadPublicKey(c.get('sso.jwtPublicKey'));
      }
      catch (e) {
        hasError(`Config key "sso.jwtPublicKey" is neither a PEM encoded public key nor the name of a file containing one - ${e.message}`);
      }
    }
  }
