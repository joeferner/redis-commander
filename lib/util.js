...
function validateConfig() {
    ...
    if (c.has(key)) {
      ...
      switch (typeof value) {
        ...
        case 'number':
          ...
          else throw new Error('config key "' + key + '" has invalid value. key must be boolean!');
          break;
        ...
        case 'string':
          switch (value.toLowerCase()) {
            ...
            case 'off':
              ...
              break;
            default:
              throw new Error('config key "' + key + '" has invalid value. key must be boolean!');
          }
          break;
  ...
  let convertNumbers = function(key) {
    if (c.has(key)) {
      ...
      switch (typeof value) {
        ...
        case 'string':
          if (isNaN(value))
            throw new Error('config key "' + key + '" has invalid value (current: ' + value + '). Key must be number!');
  ...
  let validateNumbers = function(key, isInteger, minValue, maxValue) {
    if (c.has(key)) {
      ...
      if (isInteger && !Number.isInteger(value))
        throw new Error('config key "' + key + '" value (current: ' + value + ') must be an integer number!');

      if (value < minValue || value > maxValue)
        throw new Error('config key "' + key + '" value (current: ' + value + ') must be in range ' + minValue + ' - ' + maxValue + '!');
    }
  };
  ...
  ['noSave', 'noLogData', 'ui.locked', 'ui.cliOpen', 'redis.flushOnImport', 'redis.useScan'].forEach(convertBoolean);
