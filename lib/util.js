...
exports.hasDeprecatedConfig = function() {
  return fs.existsSync(getDeprecatedConfigPath());
};

exports.getDeprecatedConfig = function (callback) {
...
exports.saveConnections = function (config, callback) {
...
exports.saveLocalConfig = function (config, callback) {
  fs.writeFile(getConfigPath('local'), JSON.stringify(config, null, 2), function (err) {
    if (typeof callback === 'function') callback(err ? err : null);
  });
};

exports.deleteConfig = function (configFile, callback) {
...
exports.deleteDeprecatedConfig = function (callback) {
...
function getConfigPath (configFile) {
  var configPath = require('config').util.getEnv('NODE_CONFIG_DIR');
  if (configFile === 'local') {
    return path.join(configPath, 'local.json');
  }
  else {  // connections
    return path.join(configPath, 'local-' + os.hostname() + '.json');
  }
}
