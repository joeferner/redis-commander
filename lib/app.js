...
function login (newConnection, callback) {
  ...
  let clientOpts = {
    ...
    family: 4,
    db: newConnection.dbIndex,
    password: newConnection.password,
    connectionName: config.get('redis.connectionName')
  };
  if (typeof newConnection.tls === 'boolean') clientOpts.tls = {};
  else if (typeof newConnection.tls === 'object') clientOpts.tls = newConnection.tls;

  let client = new Redis(clientOpts);
  client.label = newConnection.label;
  ...
  function selectDatabase () {
    ...
    return client.select(newConnection.dbIndex, function (err) {
      ...
      console.log('Redis Connection ' + newConnection.host + ':' + newConnection.port +
        (newConnection.tls ? ' with TLS' : '') +
        ' using Redis DB #' + newConnection.dbIndex);
