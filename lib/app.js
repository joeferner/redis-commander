...
function jwtVerifySso(token) {
  return new Promise(resolve => {
    ...
    if (config.get('sso.subject')) {
      verifyOpts.subject = config.get('sso.subject');
    }
    // key to validate the signature with is either a shared secret, a configured public key
    // or one downloaded from the JWKS endpoint of the identity provider
    jwt.verify(token, ssoKeys.ssoKeyResolver, verifyOpts, (err, decodedToken) => {
      if (err) {
