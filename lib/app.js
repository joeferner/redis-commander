...
module.exports = function (_redisConnections) {
  ...
  app.locals.foldingCharacter = config.get('foldingChar');
  app.locals.httpAuthDisabled = (!config.get('httpAuth.username') || !(config.get('httpAuth.passwordHash') || config.get('httpAuth.password')));
  app.locals.jwtSecret = config.get('httpAuth.jwtSecret') || crypto.randomBytes(20).toString('base64');
  app.locals.sf = sf;
  ...
  app.post(`${urlPrefix}/signin`, function(req, res, next) {
    ...
    .then(() => {
      ...
      if (req.body && (req.body.username || req.body.password)) {
        ...
        let validUser = true;
        let validPass = false;
        if (String(req.body.username) !== String(config.get('httpAuth.username'))) {
          validUser = false;
        }
        if (config.get('httpAuth.passwordHash')) {
          validPass = bcrypt.compare(String(req.body.password), String(config.get('httpAuth.passwordHash')))
        }
        else {
          // prevent empty passwords
          validPass = equalStrings(String(req.body.password) + 'X', String(config.get('httpAuth.password')) + 'X');
        }
