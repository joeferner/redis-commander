...
function login (newConnection, callback) {
  ...
  function selectDatabase () {
    ...
    return client.select(newConnection.dbIndex, function (err) {
      ...
      console.log('Redis ' + (newConnection.sentinels ? 'Sentinel ' ? '') + 'Connection ' + (opt.path ? opt.path : opt.host + ':' + opt.port) +
        (opt.tls || opt.sentinelTLS ? ' with TLS' : '') + ' using Redis DB #' + opt.db);
